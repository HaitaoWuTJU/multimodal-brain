...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="/home/wht/multimodal_brain/src/tasks/base/configs/new.yaml",
    ...
    logger = TensorBoardLogger(config['save_dir'], name="VAE_finetune_new")
    ...
    trainer = Trainer(strategy=DDPStrategy(find_unused_parameters=True),callbacks=[checkpoint_callback],max_epochs=config['train']['epoch'], devices=[5],accelerator='cuda',logger=logger)
