...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="/home/wht/multimodal_brain/src/tasks/base/configs/subject_cls.yaml",
    ...
    logger = TensorBoardLogger(config['save_dir'], name="Subject_cls")
    ...
    trainer = Trainer(strategy=DDPStrategy(find_unused_parameters=True),callbacks=[checkpoint_callback],max_epochs=config['train']['epoch'], devices=find_usable_cuda_devices(1),accelerator='cuda',logger=logger)
