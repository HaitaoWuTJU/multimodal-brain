...
def load_model(config,test_loader):
    ...
    for k,v in config['models'].items():
        if k == 'generation':
            ...
            ckpt = '/home/wht/multimodal_brain/src/tasks/pretrain/exp/pretrain/version_11/checkpoints/epoch=49-step=16200.ckpt'
...
class PLModel(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        # img_latent = self.generation.image_encoder(img)
        img_latent = img_features
        
        gen_img_latent = self.generation.denoise(debia_latent)
        
        gen_img = self.generation.decode(gen_img_latent)
        
        return eeg_latent, debia_latent, img_latent, gen_img_latent, gen_img
