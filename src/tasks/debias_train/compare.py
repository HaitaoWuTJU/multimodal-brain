...
class PLModel(pl.LightningModule):
    def __init__(self, model,config,test_loader):
        ...
        self.z_dim = self.config['models']['eeg_moving']['params']['z_dim']
    ...
    def forward(self, batch,sample_posterior=False):
        ...
        res, moving_mean = self.eeg_sd(eeg)
        eeg_latent_moving = self.eeg_moving(moving_mean)[:,:self.z_dim]
        eeg_latent_res = self.eeg_res(res)[:,:self.z_dim]
        eeg_latent = 0.5*eeg_latent_moving + 0.5*eeg_latent_res
        logit_scale = self.eeg_res.logit_scale
        clip_loss = self.criterion(eeg_latent, img_features, logit_scale)
        
        # kl_loss = torch.sum(posterior.kl())/ eeg.shape[0]

        loss = clip_loss #+ self.kl_weight * kl_loss
        return eeg_latent, eeg_latent, img_features, loss
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="/home/wht/multimodal_brain/src/tasks/base/configs/compare.yaml",
    ...
    logger = TensorBoardLogger(config['save_dir'], name="Blur", version=f"{config['data']['subjects']}")
    ...
    trainer = Trainer(log_every_n_steps=10, strategy=DDPStrategy(find_unused_parameters=True),callbacks=[checkpoint_callback],max_epochs=config['train']['epoch'], devices=[1],accelerator='cuda',logger=logger)
