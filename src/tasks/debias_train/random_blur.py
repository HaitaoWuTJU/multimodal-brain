...
class PLModel(pl.LightningModule):
    ...
    def forward(self, batch,sample_posterior=False):
        ...
        img_features_blur = self.vlmodel.encode_image(img)
        clip_loss = self.criterion(debia_latent, img_features_blur, logit_scale)
        
        # kl_loss = torch.sum(posterior.kl())/ eeg.shape[0]

        loss = clip_loss #+ self.kl_weight * kl_loss
        return eeg_latent, debia_latent, img_features_blur, loss
...
def main():
    ...
    logger = TensorBoardLogger(config['save_dir'], name="dynamic_blur", version=f"{config['data']['subjects']}_{config['ksize']}_{config['sigmaX']}")
