...
def main():
    ...
    test_dataset = SingleEEGDataset(data_dir, subject=subject, mode='test',selected_ch=config['selected_ch'],model_type=config['model_type'],average=True)
    train_dataset = SingleEEGDataset(data_dir, subject=subject, mode='train',selected_ch=config['selected_ch'],model_type=config['model_type'])
...
if __name__=="__main__":
    ...
    main()
    end_time = time.time()
    logging.info(f"时间: {end_time - start_time} 秒")