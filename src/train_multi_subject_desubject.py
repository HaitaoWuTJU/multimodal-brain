...
def train_subject(config,data,model,args):
    ...
    for epoch in range(args['epochs']):
        ...
        for i,sample in enumerate(data['train']):
            ...
            eeg_img_clip_loss = clip_loss(eeg_semantic, img_features,logit_scale)
            eeg_text_clip_loss = clip_loss(eeg_semantic, text_features,logit_scale)
            text_img_clip_loss = clip_loss(text_features, img_features,logit_scale)
            
            eeg_img_mse_loss = mse_loss(eeg_semantic, img_features)
            eeg_text_mse_loss = mse_loss(eeg_semantic, text_features)
            text_img_mse_loss = mse_loss(text_features, img_features)
            
            subject_cls_logits = model['aux'](eeg_semantic.detach())
            predicted_labels = torch.argmax(subject_cls_logits, dim=1)
            cls_accuracy = (predicted_labels == subject).float().mean().item()

            subject_cls_loss = ce_loss(subject_cls_logits,subject)

            args['optimizer'].zero_grad()
            subject_cls_loss.backward()
            args['optimizer'].step()
            
            # GAN_loss = -ce_loss(semantic_logits,subject)
            subject_cls_logits = model['aux'](eeg_semantic)
            target_tensor = torch.full((eeg_semantic.shape[0], 10), 0.1, device=device)
            GAN_loss = ce_loss(subject_cls_logits,target_tensor)
            loss = 10*eeg_img_clip_loss +  0.01*GAN_loss #+ 0.1*eeg_img_mse_loss
            args['optimizer'].zero_grad()
            loss.backward()
            args['optimizer'].step()
            ...
            similarity = (eeg_semantic @ img_features.T)
            ...
            if (i+1) % 10 == 0:
                logging.info(f"Epoch {epoch}, Iteration {i}, CLIP:{eeg_img_clip_loss.item():.4f} MSE:{eeg_img_mse_loss.item():.4f} CLS:{subject_cls_loss.item():.4f} GAN:{GAN_loss.item():.4f} ACC:{cls_accuracy:.4f}")
    ...
    for i,sample in enumerate(data['test']):
        ...
        if n_way:
            for j in range(img_features.shape[0]):
                ...
                similarity_n_way = (eeg_semantic[j] @ all_image_features_n_way.T)
...
def main():
    ...
    config = {
        ...
        "exp_root":'./exp',
        "device":device,
        "name": 'MultilabelClipLoss_GAN_Semantic_Disentagled_projectMLP_BackboneTransformer_lr1e-4_bs256',#os.path.basename(__file__).rsplit('.',1)[0],
        "lr": 1e-4,
        "epochs": 25,
        "batch_size": 256,
        ...
        # "eeg":{'name':'MLP','args':{'input_dim':len(selected_ch)*250,'output_dim':latend_dim,'hiden_dims':[]}},#{'name':'LSTMModel', 'args':{}},
