...
for i,(n_components, n_neighbors) in enumerate(cartesian_product):
    ...
    model_prediction = model.fit_predict(data_pca)
    ...
    for label in set(model_prediction):
        indices = model_prediction == label
...
plt.savefig('../results/UMAP_Embedding_pca(gmm)_intersection.pdf', format='pdf', bbox_inches='tight')
plt.savefig('../results/UMAP_Embedding_pca(gmm)intersection.png', bbox_inches='tight')