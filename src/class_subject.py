...
from dataset.things_eeg import SingleEEGDataset,EEGDataset
...
def train_subject(config,data,model,train_args,eval_args):
    ...
    for epoch in range(train_args['epochs']):
        ...
        for i,sample in enumerate(data['train']):
            ...
            loss = criterion(eeg_logits, subject)
...
def main():
    ...
    config = {
        ...
        "model_type":'ViT-B/32',
        "logger": True,
        "subjects":args.subjects,#['sub-01','sub-02','sub-03', 'sub-04', 'sub-05', 'sub-06', 'sub-07', 'sub-08', 'sub-09', 'sub-10'],
        ...
        "linear_probe":{'name':"MLP","args":{'input_dim':latend_dim,'output_dim':len(args.subjects),'hiden_dims':[]}},
    }
    ...
    test_dataset = EEGDataset(data_dir='/dev/shm/datasets/things-eeg-small/Preprocessed_data_250Hz',subjects=subjects,model_type='ViT-B/32',mode='test')
    train_dataset = EEGDataset(data_dir='/dev/shm/datasets/things-eeg-small/Preprocessed_data_250Hz',subjects=subjects,model_type='ViT-B/32',mode='train')
    ...
    train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True, drop_last=True, num_workers=12, pin_memory=True)
