...
def train_subject(config,data,model,args):
    ...
    for epoch in range(args['epochs']):
        ...
        for i,sample in enumerate(data['train']):
            ...
            similarity = (eeg_features @ img_features.T)
    ...
    for i,sample in enumerate(data['test']):
        ...
        if n_way:
            for j in range(img_features.shape[0]):
                ...
                similarity_n_way = (eeg_features[j] @ all_image_features_n_way.T)
        ...
        similarity = (eeg_features @ all_image_features.T)
...
def main():
    ...
    model = {
        'eeg':globals()[config['eeg_model']['name']](**config['eeg_model']['args']).to(device),
        # 'img':img_model,
        # 'text':text_model
    }
