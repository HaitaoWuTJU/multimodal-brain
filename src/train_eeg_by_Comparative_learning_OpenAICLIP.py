...
from eegdatasets_leaveone_OpenAICLIP import EEGDataset
...
def train_subject(config,data,model,train_args,eval_args):
    ...
    for epoch in range(train_args['epochs']):
        ...
        for i,sample in enumerate(data['train']):
            ...
            if (i+1) % 10 == 0:
                logging.info(f"Epoch {epoch + 1}, Iteration {i}, Loss: {loss.item():.5f} text_img_loss:{text_img_loss.item():.5f}")
...
def eval_subject(config,data,model,eval_args):
    ...
    for i,sample in enumerate(data['test']):
        eeg_data, labels, text, text_features, img, img_features = sample
        ...
        for j in range(img_features_project.shape[0]):
            ...
            all_true_labels_n_way.append(j)

        similarity = (eeg_features @ img_features_project.T)
        predictions = similarity.argmax(dim=-1)
        all_predicted_classes.extend(predictions.cpu().numpy())
        all_true_labels.extend(labels.numpy())
    # logging.info(all_predicted_classes)
    # logging.info("\n")
    # logging.info(all_true_labels)
    ...
    logging.info(f"Test ACC: {accuracy:.4f} accuracy_{n_way}_way:{accuracy_n_way:.4f}")
...
def main():
    ...
    config = {
        ...
        "eeg_model": {'name':'MLP',"args":{'input_dim':len(selected_ch)*250,'output_dim':768,'hiden_dims':[]}},
