...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None, selected_ch=None):
        ...
        self.chose_sample(os.path.join(self.data_path,'../','sub-01_StimulusMetadata.csv'))
        print(self.data.shape,len(self.labels), len(self.text), len(self.img))
    ...
    def chose_sample(self,path):
        df = pd.read_csv(path)
        if self.train:
            test_trials = df[df['trial_type'] == 'train']['stimulus']
            print(test_trials)
            img_list = [ele.rsplit('/',1)[-1] for ele in self.img]
            print(img_list)
            df = pd.DataFrame(img_list, columns=['image_path'])
            df.to_csv(os.path.join(self.data_path,'../','img_train.csv'), index=False)

            test_trials_set = set(test_trials)
            img_list = set(img_list)

            included_elements = list(test_trials_set.intersection(img_list))

            print(included_elements,len(included_elements))
            df = pd.DataFrame(included_elements, columns=['Img'])

            df.to_csv('train_union.csv', index=False)
...
if __name__ == "__main__":
    ...
    selected_ch = ['Fp1', 'Fp2', 'AF7', 'AF3', 'AFz', 'AF4', 'AF8', 'F7', 'F5', 'F3',
                                        ...
                                        'O1', 'Oz', 'O2']
    config = {
        "data_path": "/root/workspace/wht/multimodal_brain/datasets/things-eeg-small/Preprocessed_data_250Hz_downloads",
        "exp_root":'./exp',
        "name": "TEST",
        "lr": 1e-4,
        "epochs": 100,
