...
def main():
    ...
    test_dataset = EEGDataset(data_dir=config['data_dir'],subjects=subjects,model_type=config['model_type'],mode='test',transform=transform,avg=True)
    train_dataset = EEGDataset(data_dir=config['data_dir'],subjects=subjects,model_type=config['model_type'],mode='train',transform=transform,avg=True)
    ...
    model['eeg'].load_state_dict(torch.load(os.path.join(config['exp_root'],'train_multi_subject',f"ckpt_{'_'.join(config['subjects'])}.pth"),map_location=config['device'])['eeg'])
